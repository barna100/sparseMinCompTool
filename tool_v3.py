...
def prepare_dfaList():
  for node in nodeList:
    ...
    item.append(node[0])
    item.append([])         # in info
    item.append([])         # out info
    item.append([])         # gen info
    dfaList.append(item)
...
def analysis_mayNZ(dataList):
  ...
  while (len(workList) > 0):
	...
	set_outInfo(outInfo, entry)
      ...
      if is_equal(get_bitVector(get_outInfo(entry)),outInfoVec) == 0:
        set_genInfo(outInfoVec,entry)
        set_outInfoVec(outInfoVec,entry)
      else:
        set_genInfo([0],entry)
        if init == 0:
  	  rmList.append(entry)
...
def remove_entry(rmList, workList):
  for entry in rmList:
    del workList[workList.index(entry)]
  return workList
...
#  return check_limit(get_lb(entry), get_ub(entry), check_condition(get_nodeCondition(entry),filter_nzEntries(get_bitVector(get_outInfo(entry)), get_array(entry)), entry),entry)
  return check_limit(get_lb(entry), get_ub(entry), check_condition(get_nodeCondition(entry),filter_nzEntries(get_genInfo(entry), get_array(entry)), entry),entry)
...
#  length = len(get_outInfo(list1[0]))
  length = len(get_genInfo(list1[0]))
  item = list1[0]
  for l in list1:
    length1 = len(get_genInfo(list1[0]))
    if length1 < length:
      length = length1
      item = l
  return item     
...
def set_outInfoVec(outInfoVec,entry):
  for indx,val in enumerate(dfaList):
    if val[0] == entry:
      dfaList[indx][2][2] = outInfoVec

def set_genInfo(genInfoVec,entry):
  for indx,val in enumerate(dfaList):
    if val[0] == entry:
      dfaList[indx][3] = genInfoVec
